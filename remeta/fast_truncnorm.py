...
class truncnorm_gen(rv_continuous):
    ...
    def _stats(self, a, b):
        ...
        # Safe evaluation of x * phi(x), with limit 0 at +/-inf
        a_pA = np.zeros(np.broadcast(a, pA).shape, dtype=np.result_type(a, pA, float))
        b_pB = np.zeros(np.broadcast(b, pB).shape, dtype=np.result_type(b, pB, float))

        mask_a = np.isfinite(a)
        mask_b = np.isfinite(b)

        # Only multiply where finite => no inf*0 is ever evaluated
        a_pA[mask_a] = (a[mask_a] * pA[mask_a])
        b_pB[mask_b] = (b[mask_b] * pB[mask_b])

        mu2 = 1.0 + (a_pA - b_pB) / d - mu * mu
        return mu, mu2, None, None
